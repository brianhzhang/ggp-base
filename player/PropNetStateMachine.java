...
public class PropNetStateMachine extends StateMachine {
	...
	public int getGoal(MachineState state, Role role)
		...
		for (Proposition p : bases) {
			Set<Component> comp = p.getInputs();
	...
	public MachineState getInitialState() {
		...
		for (GdlSentence s : bases.keySet()) {
			Set<Component> comp = bases.get(s).getInputs();
			for (Component c : comp) {
				if (c.getValue()) {
					nexts.add(s);
					break;
	...
	public List<Proposition> getOrdering()
	{
		...
		for (Proposition p : propositions) {
			if (basePropositions.get(p.getName()) == null &&
					inputPropositions.get(p.getName()) == null) {
				propTemp.add(p);
			}
		}
		return new ArrayList<Proposition>(propTemp);
//		propositions = propTemp;
//		Set<Proposition> marked = new HashSet<Proposition>();
//		while (true) {
//			Proposition p = null;
//			for (Proposition temp : propositions) {
//				if (!marked.contains(temp)) {
//					p = temp;
//				}
//			}
//			if (p == null) break;
//			if (!visit(p, order, marked, new HashSet<Proposition>())) {
//				System.out.println("Fail.");
//				break;
//			}
//		}
//		for (int i = 0; i < order.size(); i ++) {
//			if (!propositions.contains(order.get(i))) {
//				order.remove(i);
//				i --;
//			}
//		}
//		System.out.println("Size of order: " + order.size() + " - Time: " + (System.currentTimeMillis() - start));
//		return order;
	}

	private boolean visit(Proposition prop, List<Proposition> order, Set<Proposition> marked, Set<Proposition> tempMarked) {
		if (!marked.contains(prop)) {
			Set<Proposition> neighbors = new HashSet<Proposition>();
			for (Component c : prop.getInputs()) {
				findChildren(c, neighbors, 0);
			}
			for (Proposition p : neighbors) {
				if (!visit(p, order, marked, tempMarked)) return false;
			}
			marked.add(prop);
			order.add(0, prop);
		}
	...
	private void findChildren(Component prop, Set<Proposition> children, int depth) {
		if (depth > 1) return; //not sure why but works perfectly when it has this line lol
		if (prop instanceof Proposition) {
			children.add((Proposition)prop);
		} else {
			for (Component c : prop.getInputs()) {
				findChildren(c, children, depth + 1);
			}
		}
	}
	...
	private List<Move> propToMoves(Set<Proposition> set, boolean any) {
		...
		for (Proposition p : set) {
			if (any) {
				moves.add(new Move(p.getName().toTerm()));
				continue;
			}
			Set<Component> comp = p.getInputs();
			for (Component c : comp) {
	...
	private void markactions (List<GdlSentence> list) {
