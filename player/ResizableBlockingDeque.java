 ...
 */
public class ResizableBlockingDeque<E>
	...
	/**
	 * Creates a {@code ResizableBlockingDeque} with a capacity of {@link Integer#MAX_VALUE}.
	 */
	public ResizableBlockingDeque() {
		this(Integer.MAX_VALUE);
	}

	/**
	 * Creates a {@code ResizableBlockingDeque} with the given (fixed) capacity.
	 *
	 * @param capacity
	 ...
	 */
	public ResizableBlockingDeque(Collection<? extends E> c) {
		this();
	 ...
	 * method {@link #offer(Object) offer}.
	 *
	 * <p>
...
//     }
	 ...
	 *
	 * @param a
	 *            the array into which the elements of the deque are to be stored, if it is big
	 ...
	 *             of every element in this deque
	 * @throws NullPointerException
	...
	/**
	 * Base class for Iterators for ResizableBlockingDeque
	 */
	private abstract class AbstractItr implements Iterator<E> {
		...
		AbstractItr() {
			...
			final ReentrantLock lock = ResizableBlockingDeque.this.lock;
			lock.lock();
			try {
				next = firstNode();
	...
	static final class LBDSpliterator<E> implements Spliterator<E> {
		...
		final ResizableBlockingDeque<E> queue;
		Node<E> current; // current node; null until initialized
		int batch; // batch size for splits
		boolean exhausted; // true when no more nodes
		long est; // size estimate

		LBDSpliterator(ResizableBlockingDeque<E> queue) {
			this.queue = queue;
			this.est = queue.size();
		}
		...
		public Spliterator<E> trySplit() {
			Node<E> h;
			final ResizableBlockingDeque<E> q = this.queue;
			int b = batch;
	...
	public void increaseCapacity() {
		if (capacity <= Integer.MAX_VALUE / 2) capacity *= 2;
	}
