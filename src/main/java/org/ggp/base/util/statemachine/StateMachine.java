...
public abstract class StateMachine {
	...
	/**
	 * Returns a list of all possible actions the role can perform in the game, regardless of
	 * legality at any particular state in the game.
	 * 
	 * @throws MoveDefinitionException
	 *             if the role has no possible moves. This indicates an error in either the game
	 ...
	 * 
	 * @throws MoveDefinitionException
	 *             if the role has no legal moves. This indicates an error in either the game
	 *             description or the StateMachine implementation.
	 */
	...
	/**
	 * Returns the state that results from the joint move given by moves in the specified state. The
	 * order of moves in moves must correspond exactly to the order of roles in the list returned by
	 * findroles().
	 * 
	 * @throws TransitionDefinitionException
	 */
	public MachineState findNext(List<Move> moves, MachineState state)
			throws TransitionDefinitionException {
	 ...
	 *             if there is no goal value or more than one goal value for the given role in the
	 *             given state. If this occurs when this is called on a terminal state, this
	 *             indicates an error in either the game description or the StateMachine
	 *             implementation.
	 */
	...
	/**
	 * Returns true if and only if the given state is a terminal state (i.e. the game is over).
	 */
	 ...
	 * <p>
	 * This method should only be called once, and it should be called before any other methods on
	 * the StateMachine.
	 */
	public abstract void initialize(List<Gdl> description);

	/**
	 * Returns the goal value for the given role in the given state. Goal values are always between
	 * 0 and 100.
	 *
	 * @throws GoalDefinitionException
	 *             if there is no goal value or more than one goal value for the given role in the
	...
	/**
	 * Returns a list of the roles in the game, in the same order as they were defined in the game
	 * description.
	 * <p>
	 * The result will be the same as calling {@link Role#computeRoles(List)} on the game rules used
	 * to initialize this state machine.
	 */
	...
	/**
	 * Returns a list containing every move that is legal for the given role in the given state.
	 *
	 * @throws MoveDefinitionException
	 *             if the role has no legal moves. This indicates an error in either the game
	 *             description or the StateMachine implementation.
	...
	/**
	 * Returns the next state of the game given the current state and a joint move list containing
	 * one move per role.
	 *
	 * @param moves
	 *            A list containing one move per role. The moves should be listed in the same order
	 *            as roles are listed by {@link #getRoles()}.
	 * @throws TransitionDefinitionException
	 *             indicates an error in either the game description or the StateMachine
	 *             implementation.
	 */
	 ...
	 */
	public MachineState getNextStateDestructively(MachineState state, List<Move> moves)
			throws TransitionDefinitionException {
		return getNextState(state, moves);
	}
	 ...
	 * This means that the state machine will only handle portions of the game tree at and below the
	 * given state; it no longer needs to properly handle earlier portions of the game tree. This
	 * constraint can be used to optimize certain state machine implementations.
	 * <p>
	 * CONTRACT: After calling this method, the state machine never deals with a state that is not
	 * "theState" or one of its descendants in the game tree.
	 */
	...
	/**
	 * Returns a list containing every joint move possible in the given state. A joint move consists
	 * of one move for each role, with the moves in the same ordering that their roles have in
	 * {@link #getRoles()}.
	 * <p>
	 * The list of possible joint moves is the Cartesian product of the lists of legal moves
	 * available for each player.
	 * <p>
	 * If only one player has more than one legal move, then the number of joint moves returned will
	 * equal the number of possible moves for that player.
	 */
	...
	/**
	 * Returns a list of every joint move possible in the given state in which the given role makes
	 * the given move. This will be a subset of the list of joint moves given by
	 * {@link #getLegalJointMoves(MachineState)}.
	 */
	...
	/**
	 * Returns a list containing every possible next state of the game after the given state. The
	 * list will contain one entry for every possible joint move that could be played; as such, a
	 * single machine state could be included multiple times.
	 */
	...
	/**
	 * Returns a map from each move that is legal for the given role in the given state to the list
	 * of possible resulting states if that move is chosen.
	 * <p>
	 * If the given role is the only role with more than one legal move, then each list of states in
	 * the map will only contain one state.
	 */
	public Map<Move, List<MachineState>> getNextStates(MachineState state, Role role)
			throws MoveDefinitionException, TransitionDefinitionException {
	...
	/**
	 * Returns a mapping from a role to the index of that role, as in the list returned by
	 * {@link #getRoles()}. This may be a faster way to check the index of a role than calling
	 * {@link List#indexOf(Object)} on that list.
	 */
	...
	/**
	 * Returns the goal values for each role in the given state. The goal values are listed in the
	 * same order the roles are listed in the game rules, which is the same order in which they're
	 * returned by {@link #getRoles()}.
	 *
	 * @throws GoalDefinitionException
	 *             if there is no goal value or more than one goal value for any one role in the
	 *             given state. If this occurs when this is called on a terminal state, this
	 *             indicates an error in either the game description or the StateMachine
	 *             implementation.
	 */
	...
	/**
	 * Returns a random joint move from among all the possible joint moves in the given state.
	 */
	...
	/**
	 * Returns a random joint move from among all the possible joint moves in the given state in
	 * which the given role makes the given move.
	 */
	...
	/**
	 * Returns a random move from among the possible legal moves for the given role in the given
	 * state.
	 */
	...
	/**
	 * Returns a state chosen at random from the possible next states of the game.
	 * <p>
	 * The distribution among states is based on the possible joint moves. This is not necessarily
	 * uniform among the possible states themselves, as multiple joint moves may result in the same
	 * state.
	 */
	...
	/**
	 * Returns a random next state of the game from the possible next states resulting from the
	 * given role playing the given move.
	 * <p>
	 * The distribution among states is based on the possible joint moves. This is not necessarily
	 * uniform among the possible states themselves, as multiple joint moves may result in the same
	 * state.
	 * <p>
	 * If the given role is the only role with more than one legal move, then there is only one
	 * possible next state for this method to return.
	 */
	public MachineState getRandomNextState(MachineState state, Role role, Move move)
			throws MoveDefinitionException, TransitionDefinitionException {
	...
	/**
	 * Returns a terminal state derived from repeatedly making random joint moves until reaching the
	 * end of the game.
	 *
	 * @param theDepth
	 *            an integer array, the 0th element of which will be set to the number of state
	 *            changes that were made to reach a terminal state.
	 */
	...
	public void getAverageDiscountedScoresFromRepeatedDepthCharges(final MachineState state,
			final double[] avgScores, final double[] avgDepth, final double discountFactor,
			final int repetitions) throws TransitionDefinitionException, MoveDefinitionException,
